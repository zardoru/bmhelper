...
public:
		...
		tracks(_tracks), combo_trk(0), button_ok(0)
	{
		...
		wxString *track_choices = new wxString[tracks.size()];
		for (size_t i=0; i<tracks.size(); i++){
		...
		combo_trk = new wxComboBox(this, ID_Tracks_Choice, track_choices[def], wxPoint(95, 5),
			wxSize(200, 15), tracks.size(), track_choices, wxCB_DROPDOWN | wxCB_READONLY);
		combo_trk->SetSelection(def);
		delete[] track_choices;
...
BEGIN_EVENT_TABLE(LoadSmfDialog, wxDialog)
...
bool MidiData::load_smf(const wxString &filename){
	...
	for (int i=0; i<16; i++) channel_mask[i] = dialog->get_channel_mask(i);
